...
def generate_plots():
    ...
    fig = px.pie(data_people, values='Количество', names='Район',
                 title='Распределение численности населения по районам',
                 hover_data=['Район'], labels={'Количество'})
    ...
    fig = px.bar(data_zone, x='Спортивная зона', y='Количество')
    plots.append(loads(dumps(fig, cls=PlotlyJSONEncoder)))
    ...
    fig = px.bar(data_sport, x='Вид спорта', y='Количество')
    plots.append(loads(dumps(fig, cls=PlotlyJSONEncoder)))
    ...
    fig = px.bar(data_sport_type, x='Вид спорта', y='Средняя площадь объектов')
    plots.append(loads(dumps(fig, cls=PlotlyJSONEncoder)))
    ...
    fig = px.bar(data_zones_type, x='Тип спортивной зоны', y='Средняя площадь спортивной зоны')
    plots.append(loads(dumps(fig, cls=PlotlyJSONEncoder)))
